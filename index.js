...
const CANDIDATE_SUBTITLES = {
  gregoire: "PS – Gauche",
  dati: "LR – Droite (+ Bournazel)",
  chikirou: "LFI – Gauche rad.",
};
...
const DEFAULT_PARAMS = {
  ret_gregoire: 0.90,
  ret_dati: 0.88,
  bournazel_gregoire: 0.25,
  bournazel_dati: 0.55,
  chikirou_gregoire: 0.20,
  ret_chikirou: 0.60,
  ret_knafo: 0.70,
  ret_mariani: 0.55,
  ret_deg: 0.75,
...
function computeResults(params) {
  const {
    ret_gregoire, ret_dati,
    bournazel_gregoire, bournazel_dati,
    chikirou_gregoire, ret_chikirou,
    ret_knafo, ret_mariani, ret_deg,
  } = params;
  ...
  const results = { gregoire: 0, dati: 0, chikirou: 0, abstention: 0 };

  results.gregoire += FIRST_ROUND.gregoire * ret_gregoire;
  results.abstention += FIRST_ROUND.gregoire * (1 - ret_gregoire);

  results.dati += FIRST_ROUND.dati * ret_dati;
  results.abstention += FIRST_ROUND.dati * (1 - ret_dati);

  results.gregoire += FIRST_ROUND.bournazel * bournazel_gregoire;
  results.dati += FIRST_ROUND.bournazel * bournazel_dati;
  results.abstention += FIRST_ROUND.bournazel * bournazel_abs;

  results.gregoire += FIRST_ROUND.chikirou * chikirou_gregoire;
  results.chikirou += FIRST_ROUND.chikirou * ret_chikirou;
  results.abstention += FIRST_ROUND.chikirou * chikirou_abs;

  results.dati += FIRST_ROUND.knafo * ret_knafo;
  results.abstention += FIRST_ROUND.knafo * (1 - ret_knafo);

  results.dati += FIRST_ROUND.mariani * ret_mariani;
  results.abstention += FIRST_ROUND.mariani * (1 - ret_mariani);

  results.chikirou += FIRST_ROUND.deg * ret_deg;
  results.abstention += FIRST_ROUND.deg * (1 - ret_deg);
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 3 }}>
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: "#1e293b", letterSpacing: "-0.01em" }}>
          {label}
          {sublabel && <span style={{ fontWeight: 400, color: "#64748b", marginLeft: 6, fontSize: 11 }}>{sublabel}</span>}
        </span>
        <span style={{
          fontFamily: "'JetBrains Mono', monospace", fontSize: 13, fontWeight: 700,
          color: warn ? "#dc2626" : color, minWidth: 48, textAlign: "right",
        }}>
          {(value * 100).toFixed(0)}%
        </span>
      </div>
      ...
      <input
        ...
        style={{
          width: "100%", marginTop: -4, appearance: "none", WebkitAppearance: "none",
          height: 22, background: "transparent", cursor: "pointer",
        }}
      />
    </div>
  );
    ...
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 5 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 700, color: "#0f172a", letterSpacing: "-0.02em" }}>
            {name}
          </span>
          <span style={{ fontSize: 11, color: "#64748b", fontWeight: 500 }}>{subtitle}</span>
        </div>
        <div style={{ textAlign: "right" }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 18, fontWeight: 800, color }}>
            {pct.toFixed(1)}%
          </span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#94a3b8", marginLeft: 8 }}>
            {Math.round(votes).toLocaleString("fr-FR")}
          </span>
        </div>
      </div>
      <div style={{ position: "relative", height: 28, borderRadius: 6, background: "#f1f5f9", overflow: "hidden" }}>
        <div style={{
          ...
          boxShadow: isWinner ? `0 2px 12px ${color}55` : "none",
        }} />
      </div>
    </div>
  );
...
function SankeyMini({ params }) {
  ...
  const bournazel_abs = Math.max(0, 1 - params.bournazel_gregoire - params.bournazel_dati);
  const chikirou_abs = Math.max(0, 1 - params.chikirou_gregoire - params.ret_chikirou);
  ...
  const flows = {
    ...
    bournazel: { gregoire: params.bournazel_gregoire, dati: params.bournazel_dati, abstention: bournazel_abs },
    chikirou: { gregoire: params.chikirou_gregoire, chikirou: params.ret_chikirou, abstention: chikirou_abs },
  ...
  const W = 700, H = 300;
  const srcX = 60, dstX = W - 60;

  const usableH = H - 40;
  const srcGap = 4;
  const srcTotalVotes = sources.reduce((a, s) => a + s.votes, 0);
  const totalSrcGap = srcGap * (sources.length - 1);
  const srcScale = (usableH - totalSrcGap) / srcTotalVotes;

  let srcY = 20;
  const srcPositions = sources.map((s) => {
    const h = Math.max(2, s.votes * srcScale);
    const pos = { ...s, y: srcY, h };
    srcY += h + srcGap;
    return pos;
  });

  const destTotals = {};
  destinations.forEach((d) => { destTotals[d] = 0; });
  sources.forEach((s) => {
    Object.entries(flows[s.key] || {}).forEach(([dest, ratio]) => {
      destTotals[dest] = (destTotals[dest] || 0) + s.votes * ratio;
    });
  });
  const totalDstVotes = destinations.reduce((a, d) => a + destTotals[d], 0);
  const dstGap = 5;
  const totalDstGap = dstGap * (destinations.length - 1);
  const dstScale = (usableH - totalDstGap) / totalDstVotes;

  let dstY = 20;
  const dstPositions = {};
  destinations.forEach((d) => {
    const h = Math.max(2, destTotals[d] * dstScale);
    dstPositions[d] = { y: dstY, h, filled: 0 };
    dstY += h + dstGap;
  });

  const paths = [];
  srcPositions.forEach((src) => {
    let srcFilled = 0;
    Object.entries(flows[src.key] || {}).forEach(([dest, ratio]) => {
      if (ratio <= 0) return;
      const flowH_src = src.h * ratio;
      const flowH_dst = dstPositions[dest].h * ((src.votes * ratio) / destTotals[dest]);
      const y1 = src.y + srcFilled;
      const y2 = dstPositions[dest].y + dstPositions[dest].filled;
      const cp = (dstX - srcX) * 0.45;
      paths.push({
        d: `M${srcX},${y1} C${srcX + cp},${y1} ${dstX - cp},${y2} ${dstX},${y2} 
            L${dstX},${y2 + flowH_dst} C${dstX - cp},${y2 + flowH_dst} ${srcX + cp},${y1 + flowH_src} ${srcX},${y1 + flowH_src} Z`,
        color: destColors[dest] || "#94a3b8",
        opacity: dest === "abstention" ? 0.15 : 0.35,
      });
      srcFilled += flowH_src;
      dstPositions[dest].filled += flowH_dst;
    });
  });

  const destLabels = { gregoire: "Grégoire", dati: "Dati", chikirou: "Chikirou", abstention: "Abstention" };
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: 700 }}>
      ...
      {srcPositions.map((s, i) => (
        <g key={`src-${i}`}>
          <rect x={srcX - 6} y={s.y} width={6} height={s.h} rx={2} fill={s.color} />
          <text x={srcX - 12} y={s.y + s.h / 2} textAnchor="end" dominantBaseline="central"
            style={{ fontSize: 10, fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fill: "#475569" }}>
            {s.label}
          </text>
        </g>
      ))}
      {destinations.map((d) => (
        <g key={`dst-${d}`}>
          <rect x={dstX} y={dstPositions[d].y} width={6} height={dstPositions[d].h} rx={2} fill={destColors[d]} />
          <text x={dstX + 14} y={dstPositions[d].y + dstPositions[d].h / 2} textAnchor="start" dominantBaseline="central"
            style={{ fontSize: 10, fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fill: "#475569" }}>
            {destLabels[d]}
          </text>
        </g>
    ...
    <div style={{
      ...
      <style>{`
        input[type=range]::-webkit-slider-thumb {
          ...
          box-shadow: 0 1px 4px rgba(0,0,0,0.25); cursor: pointer; margin-top: -5px;
        }
      ...
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: "#94a3b8", marginBottom: 4 }}>
            Simulation · Second tour
          </div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "#0f172a", margin: 0, letterSpacing: "-0.03em", lineHeight: 1.1 }}>
            Municipales Paris 2026
          </h1>
          <div style={{ fontSize: 12, color: "#64748b", marginTop: 6 }}>
            Modèle de transfert de voix · 3 qualifiés · Bournazel fusionné avec Dati · 9 paramètres
          </div>
        </div>
        ...
        <div style={{
          ...
          {QUALIFIED
            ...
            .map((c) => (
              <ResultBar
                key={c} name={CANDIDATE_LABELS[c]} subtitle={CANDIDATE_SUBTITLES[c]}
                votes={results[c]} totalExpressed={totalExpressed} maxVotes={maxVotes}
                color={CANDIDATE_COLORS[c]} isWinner={c === winner}
              />
            ))}
        </div>
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", margin: "0 0 8px 0" }}>
            Flux de transfert de voix
          </h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div style={{
            ...
            <h3 style={{ fontSize: 12, fontWeight: 700, color: "#64748b", margin: "0 0 14px 0", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              Rétention – Qualifiés
            </h3>
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", marginBottom: 8 }}>
              BOURNAZEL — fusionné, 2 destinations
              <span style={{ marginLeft: 6, fontSize: 9, padding: "1px 6px", background: "#dbeafe", color: "#3b82f6", borderRadius: 8, fontWeight: 700 }}>
                fusion Dati
              </span>
            </div>
            ...
            <div style={{ fontSize: 11, color: bournWarn ? "#dc2626" : "#94a3b8", fontWeight: 500 }}>
              → Abstention implicite : {(bournAbs * 100).toFixed(0)}% {bournWarn && "⚠ nul !"}
            </div>
            ...
            <div style={{ fontSize: 11, color: chikWarn ? "#dc2626" : "#94a3b8", fontWeight: 500 }}>
              → Abstention implicite : {(chikAbs * 100).toFixed(0)}% {chikWarn && "⚠ nul !"}
            </div>
          </div>
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 12, fontWeight: 700, color: "#64748b", margin: "0 0 14px 0", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              Report – Non qualifiés
            </h3>
            <Slider label="Knafo → Dati" sublabel="(reste → abst.)" value={params.ret_knafo}
              onChange={(v) => setParam("ret_knafo", v)} color={CANDIDATE_COLORS.dati} />
            <Slider label="Mariani → Dati" sublabel="(reste → abst.)" value={params.ret_mariani}
              onChange={(v) => setParam("ret_mariani", v)} color={CANDIDATE_COLORS.dati} />
            <Slider label="DEG → Chikirou" sublabel="(reste → abst.)" value={params.ret_deg}
              onChange={(v) => setParam("ret_deg", v)} color={CANDIDATE_COLORS.chikirou} />

            <div style={{ borderTop: "1px solid #e2e8f0", marginTop: 18, paddingTop: 16 }}>
              <h3 style={{ fontSize: 12, fontWeight: 700, color: "#64748b", margin: "0 0 12px 0", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.name} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      {row.vals.map((v, i) => (
                        <td key={i} style={{
                          padding: "5px 4px", textAlign: "right", fontSize: 11,
                          color: v === 0 ? "#cbd5e1" : v > 0.5 ? "#0f172a" : "#64748b",
        ...
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 10, color: "#94a3b8" }}>
          Modèle simplifié — ne constitue pas une prédiction électorale
        </div>
      </div>
    </div>
  );
